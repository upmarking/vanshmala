...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // ⚡ Bolt: Batch-fetch profiles for all comments across all posts to solve N+1 query issue
            ...
            if (allCommentProfileIds.size > 0) {
                const profileIdsArr = Array.from(allCommentProfileIds);
                // Chunk the profile ids if the size is very large (Supabase limit)
                // Assuming typical feed sizes don't exceed reasonable limits per request,
                // but we batch fetch all of them in a single query here.
                const { data: globalProfileData } = await supabase
                    .from('profiles')
                    .select('id, full_name, avatar_url')
                    .in('id', profileIdsArr);

                if (globalProfileData) {
                    globalProfileData.forEach((p: any) => {
            ...
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach profile info to each comment using the globally batched profile map
