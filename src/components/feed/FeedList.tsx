...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // BOLT OPTIMIZATION: Extract unique profile IDs across ALL posts to prevent N+1 queries.
            const allCommentProfileIds = new Set<string>();
            (data as any[]).forEach(post => {
                const comments: FeedComment[] = Array.isArray(post.comments) ? post.comments : [];
                comments.forEach(c => {
                    if (c.profile_id) {
