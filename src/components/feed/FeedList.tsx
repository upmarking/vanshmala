...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // Extract all unique profile_ids from comments across all posts to prevent N+1 queries
            const allCommentProfileIds = [...new Set(
                (data as any[]).flatMap(post =>
                    Array.isArray(post.comments) ? post.comments.map((c: any) => c.profile_id) : []
                )
            )];

            let globalProfileMap: Record<string, { full_name: string | null; avatar_url: string | null }> = {};

            if (allCommentProfileIds.length > 0) {
                const { data: profileData } = await supabase
                    .from('profiles')
                    .select('id, full_name, avatar_url')
                    .in('id', allCommentProfileIds);
            ...
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach profile info to each comment from the globally fetched map
