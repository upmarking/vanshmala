...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // ⚡ Bolt: Extract unique profile_ids across all comments in all posts to prevent N+1 queries
            ...
            // ⚡ Bolt: Single batched query to fetch all required profiles
            let globalProfileMap: Record<string, { full_name: string | null; avatar_url: string | null }> = {};
            const uniqueProfileIds = Array.from(allProfileIds);
            ...
            // ⚡ Bolt: Synchronous map using the populated dictionary
