...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // Previously, profile data for comments was fetched inside a .map() loop, creating an N+1 query issue.
            // Now, we collect all unique profile IDs from comments across ALL posts first,
            // fetch them in a single batched .in() query, and use a dictionary for O(1) synchronous lookups.

            // 1. Collect all unique profile IDs from all comments
            ...
            // 2. Batch fetch profiles in a single query
            ...
            // 3. Synchronously map over posts, using the O(1) lookup map
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach profile info to each comment using the map
