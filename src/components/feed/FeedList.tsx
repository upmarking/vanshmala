...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // BOLT OPTIMIZATION: Extract unique profile IDs across ALL posts to prevent N+1 query problem
            const allProfileIds = [...new Set(
                (data as any[]).flatMap(post => {
                    const comments = Array.isArray(post.comments) ? post.comments : [];
                    return comments.map(c => c.profile_id);
                })
            )];
            ...
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach profile info to each comment using the globally batched map
