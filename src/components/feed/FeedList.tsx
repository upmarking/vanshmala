...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // ⚡ Bolt: Collect all unique profile IDs from all comments across all posts
            ...
            // ⚡ Bolt: Fetch all needed profiles in a single batched query to fix N+1 problem
            ...
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach profile info to each comment synchronously using pre-fetched profileMap
