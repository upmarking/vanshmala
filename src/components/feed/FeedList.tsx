...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // ⚡ BOLT OPTIMIZATION: Extract unique profile_ids from all comments across all posts
            // to avoid N+1 queries when mapping over posts.
            const allComments = (data as any[]).flatMap(post => Array.isArray(post.comments) ? post.comments : []);
            const allProfileIds = [...new Set(allComments.map(c => c.profile_id))];
            ...
            // Map posts synchronously since we pre-fetched all required profiles
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach pre-fetched profile info to each comment
