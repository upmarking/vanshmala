...
export const FeedList = ({ refreshTrigger, filterType = "all" }: FeedListProps) => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // Collect all unique profile_ids from all comments across all posts
            const allProfileIds = new Set<string>();
            (data as any[]).forEach(post => {
                const rawComments: FeedComment[] = Array.isArray(post.comments) ? post.comments : [];
                rawComments.forEach(c => allProfileIds.add(c.profile_id));
            });

            // Batch-fetch profile info for all comments at once (Fix N+1 query problem)
            ...
            const formattedPosts: FeedPost[] = (data as any[]).map((post) => {
                ...
                // Attach profile info to each comment using the pre-fetched global profile map
